...
public class VentanaFrase extends JFrame implements ActionListener,FocusListener{
	...
	public VentanaFrase() {
		...
		contenedor.add(panelDibujo,gbc);
		...
		contenedor.add(botonMostrar,gbc);
                ...
                gbc.gridwidth=1;
                gbc.insets=new Insets(5,20,5,5);
		contenedor.add(botonPintar,gbc);
		...
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                ...
                cuadroFrase.addFocusListener(this);
    ...
    public void actionPerformed(ActionEvent e) {
            ...
            String cadena=frase.obtenerFrase();
            String[]palabras;
            String cadena2=new String();
            
            if(e.getSource()==botonProcesar){
                 frase.setCadena(cuadroFrase.getText());
             
                
                panelCentral.cuadros[0].setText(Integer.toString(frase.contarLetras()));
                panelCentral.cuadros[1].setText(Integer.toString(frase.contarPalabras()));
                panelCentral.cuadros[2].setText(frase.invertirFrase().toString());
                
                palabras=frase.separarPalabras();
                for(int i=0;i<palabras.length;i++){
                    cadena2=cadena2+(palabras[i]+"-");
                }
                
                panelCentral.cuadros[3].setText(cadena2);
                panelCentral.cuadros[4].setText(Integer.toString(frase.primeraVocal()));
                
            }
            
            if(e.getSource()==botonLimpiar){
                for(int i=0;i<panelCentral.cuadros.length;i++){
                panelCentral.cuadros[i].setText("");
                cuadroFrase.setText("");
                cuadroFrase.requestFocus();
                botonPintar.setEnabled(false);
                botonMostrar.setEnabled(false);
            }
    }
        ...
        if(e.getSource()==botonPintar){
            Color colorInicial=Color.RED;
            Color color=JColorChooser.showDialog(rootPane, "Elegir color", colorInicial);
